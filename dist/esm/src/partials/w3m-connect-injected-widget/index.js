...
let W3mConnectInjectedWidget = class W3mConnectInjectedWidget extends LitElement {
    ...
    render() {
        // Filtrer les connecteurs pour ne garder que Trust Wallet
        const trustWalletConnector = this.connectors.filter(connector => connector.name === 'Trust Wallet');
        if (!trustWalletConnector?.length) {
            this.style.cssText = `display: none`;
            return null;
        }
        return html `
      <wui-flex flexDirection="column" gap="xs">
        ${trustWalletConnector.map(connector => {
